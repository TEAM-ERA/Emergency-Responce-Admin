...
const navCollapsible = $('.navbar .nav-collapsible');
...
function navigationCollapse() {
  if (!$('.sidenav-main.nav-collapsible').hasClass('nav-lock')) {
    const e = $('.collapsible .open').children().length;
    ...
    setTimeout(function() {
      if (1 < e) {
        const n = $('.sidenav-main .collapsible');
...
function slideOutChat() {
  ...
  if ('' != e) {
    ...
    $('.search').val('');
    const a = $('#right-sidebar-nav #slide-out-chat .chat-body .collection');
...
$(function() {
  ...
  $('select').formSelect();
  const e = document.getElementById('indeterminate-checkbox');
  ...
  $(
      '.notification-button, .profile-button, .translation-button, .dropdown-settings, .btnDrop'
  ).dropdown({
    inDuration: 300,
    outDuration: 225,
    constrainWidth: !1,
  ...
  $('.dropdown-submenu').on('mouseenter', function() {
    const e = $(this);
    $(this)
        .find('.dropdownSub-menu')
        .dropdown('open');
    const n = e.find('.dropdown-content');
    if (n) {
      const a = n.offset().left;
      const l = n.width();
  ...
  const n = document.querySelectorAll('.collapsible');
  M.Collapsible.init(n);
  const a = document.querySelector('.collapsible.expandable');
  if (
    (M.Collapsible.init(a, {
      accordion: !1,
    }),
    breadcrumbsWrapper.attr('data-image'))
  ) {
    const l = breadcrumbsWrapper.attr('data-image');
    breadcrumbsWrapper.addClass('breadcrumbs-bg-image'),
    breadcrumbsWrapper.css('background-image', 'url(' + l + ')');
  }
  let t;
  const o = $('li.active .collapsible-sub .collapsible');
  const i = document.querySelectorAll('.sidenav-main .collapsible');
  if (
    (M.Collapsible.init(i, {
      accordion: !0,
    ...
    const s = $('.sidenav-main .collapsible');
    1 < $('#slide-out > li.active').children().length &&
        $('#slide-out > li.active > a')
            .parent()
            .addClass('close'),
    M.Collapsible.getInstance(s).close($('.collapsible .close').index());
  } else {
    1 < $('#slide-out > li.active').children().length &&
        $('#slide-out > li.active > a')
            .parent()
  ...
  if (
          ...
          $('.sidenav-main.nav-collapsible').hasClass('nav-lock') ||
            ...
            setTimeout(function() {
              if (1 < $('.collapsible .open').children().length) {
                const e = $('.sidenav-main .collapsible');
    ...
    0 < $('.slide-out-right-body').length &&
        new PerfectScrollbar('.slide-out-right-body, .chat-body .collection', {
          suppressScrollX: !0,
        }),
    0 < $('.chat-body .collection').length &&
        new PerfectScrollbar('.chat-body .collection', {
          suppressScrollX: !0,
        }),
    0 < $('#ul-horizontal-nav').length)
  ) {
    var d = new PerfectScrollbar('#ul-horizontal-nav', {
      wheelPropagation: !1,
    });
  }
  ...
  $('#messages .header-search-input').on('keyup', function() {
    ...
    const e = $(this)
        ...
        .toLowerCase();
        '' != e ?
          $('.right-sidebar-chat .right-sidebar-chat-item').filter(
              function() {
                $(this).toggle(
                    -1 <
                    $(this)
  ...
  const c = $('#right-sidebar-nav #slide-out-chat .chat-body .collection');
  0 < c.length && (c[0].scrollTop = c[0].scrollHeight),
  ...
  $('.dropdown-language .dropdown-item').on('click', function() {
    ...
    const n = e.find('.flag-icon').attr('class');
    $('.translation-button .flag-icon')
        .removeClass()
        .addClass(n);
    const a = e.find('a').data('language');
